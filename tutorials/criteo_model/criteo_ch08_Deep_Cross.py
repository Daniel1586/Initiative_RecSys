...
Implementation of DCN model with the following features：
...
def main(_):
    ...
    if FLAGS.file_name == "":       # 存储算法模型文件名称[标记不同时刻训练模型,程序执行日期前一天:20190327]
        FLAGS.file_name = "ch08_DCN_" + (date.today() + timedelta(-1)).strftime('%Y%m%d')
    ...
    print("==================== 2.Set model params and Build DCN model...")
    model_params = {
    ...
    print("==================== 3.Apply DCN model to diff tasks...")
    ...
    if FLAGS.task_mode == "train":
        ...
        dcn.export_savedmodel(FLAGS.serve_dir, serving_input_receiver_fn)
