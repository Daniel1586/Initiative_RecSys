...
class FeatureDictionary(object):
    def __init__(self, trainfile=None, testsfile=None, dftrain=None,
                 dftests=None, numeric_cols=None, ignore_cols=None):
        assert not ((trainfile is None) and (dftrain is None)), "trainfile or dfTrain at least one is set"
        assert not ((trainfile is not None) and (dftrain is not None)), "only one can be set"
        assert not ((testsfile is None) and (dftests is None)), "testfile or dfTest at least one is set"
        assert not ((testsfile is not None) and (dftests is not None)), "only one can be set"
...
class DataParser(object):
    ...
    def parse(self, infile=None, df=None, has_label=False):
        ...
        if has_label:
            return cate_xi, cate_xv, y
        else:
            return cate_xi, cate_xv, ids
