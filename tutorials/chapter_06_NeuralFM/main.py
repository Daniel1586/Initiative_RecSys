...
def run_base_model_nfm(dftrain, dftests, folds, nfm_para):
    ...
    nfm_para['feature_size'] = fd.feature_dim
    nfm_para['field_size'] = len(_xi_train[0])

    y_tests_meta = np.zeros((dftests.shape[0], 1), dtype=float)
    _get = lambda x, l: [x[i1] for i1 in l]
    results_epoch_train = np.zeros((len(folds), nfm_para['epoch']), dtype=float)
    results_epoch_valid = np.zeros((len(folds), nfm_para['epoch']), dtype=float)
    ...
    for i, (train_idx, valid_idx) in enumerate(folds):
        ...
        # 模型建立/训练/预测
        nfm = NeuralFM(**nfm_para)
        nfm.fit(xi_train, xv_train, y_train_, xi_valid, xv_valid, y_valid_)

        y_tests_meta += nfm.predict(_xi_tests, _xv_tests, _id_test)
        results_epoch_train[i] = nfm.train_result
        results_epoch_valid[i] = nfm.valid_result

    y_tests_meta /= float(len(folds))

    # save result
    clf_str = "NeuralFM"
    print("%s: %d Epoch" % (clf_str, nfm_para['epoch']))
    filename = "%s_%dEpoch.csv" % (clf_str, nfm_para['epoch'])
...
print('========== 3.Running base model NFM...')
y_test_nfm = run_base_model_nfm(dfTrain, dfTests, kfolds, nfm_params)
