...
class CategoryDictGenerator:
    ...
    def build(self, datafile, categorical_feature, cutoff=0):
        ...
        for j in range(0, self.num_feature):
            # 剔除频次小于cutoff的特征,剩下特征按频次从大到小排序
            self.dicts[j] = filter(lambda x: x[1] >= cutoff, self.dicts[j].items())
            self.dicts[j] = sorted(self.dicts[j], key=lambda x: (-x[1], x[0]))
            # 符合条件的categorical特征, 编号1:len()-1, 不符合条件的特征编号为0
            vocabs, _ = list(zip(*self.dicts[j]))
            self.dicts[j] = dict(zip(vocabs, range(1, len(vocabs) + 1)))
            self.dicts[j]['<unk>'] = 0
...
def preprocess(datadir, outdir):
    ...
    with open(FLAGS.output_dir + 'tests.set', 'w') as out:
